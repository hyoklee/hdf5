...
test_mdset(size_t niter, unsigned flags, hid_t fapl_id)
{
    ...
    for (i = 0; i < niter; i++) {
        ...
        ndsets = (flags & MDSET_FLAG_MLAYOUT) ? 6
                 : (flags & MDSET_FLAG_MDSET) ? (size_t)(arc4random() % max_dsets) + 1
                                              : 1;
        ...
        for (j = 0; j < ndsets; j++) {
            ...
            dset_dims[j][0] = (hsize_t)(arc4random() % MAX_DSET_X + 1);
            dset_dims[j][1] = (hsize_t)(arc4random() % MAX_DSET_Y + 1);
             ...
             * 1 and 2) */
            if (use_chunk) {
                chunk_dims[0] = (hsize_t)(arc4random() % MAX_CHUNK_X + 1);
                chunk_dims[1] = (hsize_t)(arc4random() % MAX_CHUNK_Y + 1);
        ...
        for (j = 0; j < OPS_PER_FILE; j++) {
            ...
            do_read = (j == 0 && flags & MDSET_FLAG_MLAYOUT) ? false : (bool)(arc4random() % 2);
            ...
            for (k = 0; k < ndsets; k++) {
                ...
                sel_type = (int)(arc4random() % 3);
                if (sel_type == 0) {
                    /* Hyperslab */
                    size_t nhs      = (size_t)(arc4random() % MAX_HS + 1); /* Number of hyperslabs */
                    ...
                    for (l = 0; l < nhs; l++) {
                        /* Generate hyperslab */
                        count[0] = (hsize_t)(arc4random() % max_hs_x + 1);
                        count[1] = (hsize_t)(arc4random() % max_hs_y + 1);
                        start[0] = (count[0] == dset_dims[k][0])
                                       ? 0
                                       : (hsize_t)arc4random() % (dset_dims[k][0] - count[0] + 1);
                        start[1] = (count[1] == dset_dims[k][1])
                                       ? 0
                                       : (hsize_t)arc4random() % (dset_dims[k][1] - count[1] + 1);
                    ...
                    /* Point selection */
                    size_t npoints = (size_t)(arc4random() % MAX_POINTS + 1); /* Number of points */

                    /* Generate points */
                    for (l = 0; l < npoints; l++) {
                        points[2 * l]       = (unsigned)(arc4random() % dset_dims[k][0]);
                        points[(2 * l) + 1] = (unsigned)(arc4random() % dset_dims[k][1]);
                    } /* end for */
