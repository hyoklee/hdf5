    ...
    do {
        ...
        for (u = 0, null_msg = &oh->mesg[0]; u < oh->nmesgs; u++, null_msg++) {
            ...
            if (H5O_NULL_ID == null_msg->type->id && null_msg->chunkno > 0 &&
                ...
                unsigned    deleted_chunkno; /* Chunk # to delete */
                unsigned    chk_u;           /* Inner loop variable for chunk index adjustment */
                unsigned    msg_u;           /* Inner loop variable for message index adjustment */
                ...
                if (null_msg->chunkno < (oh->nchunks - 1)) {
                    ...
                    for (chk_u = null_msg->chunkno; chk_u < (oh->nchunks - 1); chk_u++) {
                        ...
                        if (H5AC_get_entry_status(f, oh->chunk[chk_u].addr, &chk_proxy_status) < 0)
                            HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                        ...
                        if (chk_proxy_status & H5AC_ES__IN_CACHE) {
                            if (H5O__chunk_update_idx(f, oh, chk_u) < 0)
                ...
                for (msg_u = 0, curr_msg = &oh->mesg[0]; msg_u < oh->nmesgs; msg_u++, curr_msg++) {
